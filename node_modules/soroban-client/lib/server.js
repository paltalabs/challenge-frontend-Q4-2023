        ...
        return _regeneratorRuntime().wrap(function _callee5$(_context5) {
          while (1) switch (_context5.prev = _context5.next) {
            case 0:
              ...
              return _context5.abrupt("return", jsonrpc.post(this.serverURL.toString(), 'getLedgerEntries', expandRequestIncludeExpirationLedgers(keys).map(function (k) {
                return k.toXDR('base64');
              })).then(function (response) {
                return mergeResponseExpirationLedgers(response, keys);
              }));
            case 2:
            case "end":
...
function mergeResponseExpirationLedgers(ledgerEntriesResponse, requestedKeys) {
  var _ledgerEntriesRespons;
  var requestedKeyXdrs = new Set(requestedKeys.map(function (requestedKey) {
    return requestedKey.toXDR('base64');
  }));
  var expirationKeyToRawEntryResult = new Map();
  ((_ledgerEntriesRespons = ledgerEntriesResponse.entries) !== null && _ledgerEntriesRespons !== void 0 ? _ledgerEntriesRespons : []).forEach(function (rawEntryResult) {
    var _expirationKeyToRawEn;
    if (!rawEntryResult.key || !rawEntryResult.xdr) {
      throw new TypeError("invalid ledger entry: ".concat(JSON.stringify(rawEntryResult)));
    }
    var parsedKey = _stellarBase.xdr.LedgerKey.fromXDR(rawEntryResult.key, 'base64');
    var isExpirationMeta = parsedKey.switch().value === _stellarBase.xdr.LedgerEntryType.expiration().value && !requestedKeyXdrs.has(rawEntryResult.key);
    var keyHash = isExpirationMeta ? parsedKey.expiration().keyHash().toString() : (0, _stellarBase.hash)(parsedKey.toXDR()).toString();
    var rawEntry = (_expirationKeyToRawEn = expirationKeyToRawEntryResult.get(keyHash)) !== null && _expirationKeyToRawEn !== void 0 ? _expirationKeyToRawEn : rawEntryResult;
    if (isExpirationMeta) {
      var expirationLedgerSeq = _stellarBase.xdr.LedgerEntryData.fromXDR(rawEntryResult.xdr, 'base64').expiration().expirationLedgerSeq();
      expirationKeyToRawEntryResult.set(keyHash, _objectSpread(_objectSpread({}, rawEntry), {}, {
        expirationLedgerSeq: expirationLedgerSeq
      }));
    } else {
      expirationKeyToRawEntryResult.set(keyHash, _objectSpread(_objectSpread({}, rawEntry), rawEntryResult));
    }
  });
  ledgerEntriesResponse.entries = _toConsumableArray(expirationKeyToRawEntryResult.values());
  return ledgerEntriesResponse;
}
function expandRequestIncludeExpirationLedgers(keys) {
  return keys.concat(keys.filter(function (key) {
    return key.switch().value !== _stellarBase.xdr.LedgerEntryType.expiration().value;
  }).map(function (key) {
    return _stellarBase.xdr.LedgerKey.expiration(new _stellarBase.xdr.LedgerKeyExpiration({
      keyHash: (0, _stellarBase.hash)(key.toXDR())
    }));
  }));
}